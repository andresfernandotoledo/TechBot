...
from techbot.camera_tester import (
    detect_cameras, test_camera, rtsp_urls, api_info,
    converter_details, pinout_info, reset_procedure,
    CONVERTER_GUIDE_STEPS, CAMERA_WORKFLOW,
    onvif_ptz, onvif_ptz_presets, camera_reboot,
    camera_factory_reset, camera_set_ip, camera_set_password,
    camera_set_datetime, camera_set_dhcp, camera_snapshot,
    camera_info_via_http,
    default_creds_for, default_ip_for,
)


@app.route("/api/camera-tester/ptz")
def api_camera_tester_ptz():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    command = request.args.get("command", "right")
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    return jsonify(onvif_ptz(host, port, user, password, command))


@app.route("/api/camera-tester/reboot")
def api_camera_tester_reboot():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    brand = request.args.get("brand", "Hikvision")
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    return jsonify(camera_reboot(host, port, user, password, brand))


@app.route("/api/camera-tester/camera-info")
def api_camera_tester_camera_info():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    ...
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    return jsonify(camera_info_via_http(host, port, user, password, brand))


@app.route("/api/camera-tester/ptz-presets")
def api_camera_tester_ptz_presets():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    ...
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    return jsonify(onvif_ptz_presets(host, port, user, password, action, preset_name))


@app.route("/api/camera-tester/set-ip")
def api_camera_tester_set_ip():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    ...
    subnet_mask = request.args.get("subnet_mask", "255.255.255.0")
    gateway = request.args.get("gateway", "")
    if not host or not new_ip:
        return jsonify({"error": "Host y new_ip requeridos"}), 400
    return jsonify(camera_set_ip(host, port, user, password, brand, new_ip, subnet_mask, gateway))


@app.route("/api/camera-tester/set-password")
def api_camera_tester_set_password():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    brand = request.args.get("brand", "Hikvision")
    new_password = request.args.get("new_password", "admin123")
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    return jsonify(camera_set_password(host, port, user, password, brand, "admin", new_password))


@app.route("/api/camera-tester/set-datetime")
def api_camera_tester_set_datetime():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    ...
    return jsonify(camera_set_datetime(host, port, user, password, brand, ntp_server=ntp_server))


@app.route("/api/camera-tester/set-dhcp")
def api_camera_tester_set_dhcp():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    ...
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    return jsonify(camera_set_dhcp(host, port, user, password, brand, enabled))


@app.route("/api/download-apk")
def api_download_apk():
    apk_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "android", "app", "build", "outputs", "apk", "debug", "app-debug.apk")
    apk_path = os.path.normpath(apk_path)
    if os.path.exists(apk_path):
        return send_file(apk_path, mimetype="application/vnd.android.package-archive", as_attachment=True, download_name="PilotEye.apk")
    # Try alternative path for PC version
    alt_path = "/home/andres/Escritorio/boot tecnico/android/app/build/outputs/apk/debug/app-debug.apk"
    if os.path.exists(alt_path):
        return send_file(alt_path, mimetype="application/vnd.android.package-archive", as_attachment=True, download_name="PilotEye.apk")
    return jsonify({"error": "APK no encontrado. Compilá primero con ./gradlew assembleDebug"}), 404


@app.route("/api/camera-tester/snapshot")
def api_camera_tester_snapshot():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    ...
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    data = camera_snapshot(host, port, user, password, brand)
    if data:
        return Response(data, mimetype="image/jpeg")
    return jsonify({"error": "No se pudo obtener snapshot"}), 502


@app.route("/api/camera-tester/factory-reset")
def api_camera_tester_factory_reset():
    host = request.args.get("host", "")
    port = request.args.get("port", 80, type=int)
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    ...
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    return jsonify(camera_factory_reset(host, port, user, password, brand))


@app.route("/api/camera-tester/default-creds")
def api_camera_tester_default_creds():
    brand = request.args.get("brand", "")
    if brand:
        return jsonify(default_creds_for(brand))
    return jsonify(api_info()["default_creds"])


@app.route("/api/camera-tester/info")
def api_camera_tester_info():
    return jsonify(api_info())


@app.route("/api/camera-tester/workflow")
def api_camera_tester_workflow():
    return jsonify(CAMERA_WORKFLOW)


@app.route("/api/camera-tester/detect")
def api_camera_tester_detect():
    subnet = request.args.get("subnet", "")
    if not subnet:
        return jsonify({"error": "Subred requerida (ej: 192.168.1.0/24)"}), 400
    timeout = request.args.get("timeout", 1.5, type=float)
    return _async_or_run(detect_cameras, subnet, timeout)


@app.route("/api/camera-tester/test")
def api_camera_tester_test():
    host = request.args.get("host", "")
    if not host:
        return jsonify({"error": "Host requerido"}), 400
    timeout = request.args.get("timeout", 2, type=float)
    return jsonify(test_camera(host, timeout))


@app.route("/api/camera-tester/rtsp-urls")
def api_camera_tester_rtsp():
    brand = request.args.get("brand", "Generic")
    ip = request.args.get("ip", "192.168.1.100")
    user = request.args.get("user", "admin")
    password = request.args.get("password", "admin")
    return jsonify({"brand": brand, "ip": ip, "urls": rtsp_urls(brand, ip, user, password)})


@app.route("/api/camera-tester/converter")
def api_camera_tester_converter():
    name = request.args.get("name", "")
    if name:
        return jsonify(converter_details(name))
    from techbot.camera_tester import CONVERTER_INFO
    return jsonify({"converters": {k: v for k, v in CONVERTER_INFO.items()}})


@app.route("/api/camera-tester/pinout")
def api_camera_tester_pinout():
    name = request.args.get("name", "")
    if name:
        return jsonify(pinout_info(name))
    return jsonify({"pinouts": ["BNC (hembra) a RJ45", "RJ45 PoE (Cámaras IP)", "Siamese (BNC + Alimentación) a RJ45"]})


@app.route("/api/camera-tester/reset")
def api_camera_tester_reset():
    brand = request.args.get("brand", "")
    return jsonify(reset_procedure(brand))


@app.route("/api/camera-tester/converter-guide")
def api_camera_tester_converter_guide():
    return jsonify({"guide": CONVERTER_GUIDE_STEPS})
...
def api_snmp_info():
    flat_mibs = {}
    for cat_key, cat_data in MIBS.items():
        for name, mib in cat_data["oids"].items():
            flat_mibs[name] = mib["oid"]
    return jsonify({
        "mibs": flat_mibs,
        "categories": {k: {"label": v["label"], "oids": {n: m["oid"] for n, m in v["oids"].items()}} for k, v in MIBS.items()},
        "howto": "SNMP v1/v2c puro (sin binarios externos)",
        "mib_details": {n: {"oid": m["oid"], "desc": m["desc"], "cat": cat} for cat, v in MIBS.items() for n, m in v["oids"].items()},
        "functions": [
